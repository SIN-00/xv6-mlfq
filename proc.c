...
exit(void)
{
  ...
  sched();
  release(&ptable.lock);
  panic("zombie exit");  // 스케줄러로 넘어가며 프로세스를 종료
}
  ...
  for(;;){
    ...
    for(int level = 0; level < 4; level++) {
      for(p = &ptable.proc[NPROC-1]; p >= ptable.proc; p--) {
        if(p->state == RUNNABLE && p->q_level == level && p->cpu_used == 0) {
          if (highest_priority_proc == 0 || p->cpu_wait < highest_priority_proc->cpu_wait) {
...
yield(void)
{
  ...
  myproc()->cpu_wait = 0;
  if (myproc()->cpu_burst >= time_quantum[myproc()->q_level]) {
    if(myproc()->q_level < 3) { 
      myproc()->q_level++;
    }
  myproc()->cpu_burst = 0;
